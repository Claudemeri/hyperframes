        ...
        <div className="flex items-center gap-1">
          ...
          {STUDIO_KEYFRAMES_ENABLED && (gsapAnimId || onCommitAnimatedProperty) && (
            <KeyframeNavigation
              ...
              onRemoveKeyframe={(pct) => { const id = idFor("z"); if (id) onRemoveKeyframe?.(id, pct); }}
              onConvertToKeyframes={() => { const id = idFor("z"); if (id) onConvertToKeyframes?.(id); }}
            />
          )}
        </div>
        <div className="flex items-center gap-1">
          ...
          {STUDIO_KEYFRAMES_ENABLED && (gsapAnimId || onCommitAnimatedProperty) && (
            <KeyframeNavigation
              ...
              onRemoveKeyframe={(pct) => { const id = idFor("scale"); if (id) onRemoveKeyframe?.(id, pct); }}
              onConvertToKeyframes={() => { const id = idFor("scale"); if (id) onConvertToKeyframes?.(id); }}
            />
          )}
        </div>
        <MetricField
