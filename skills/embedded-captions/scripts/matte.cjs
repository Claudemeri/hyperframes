...
const MODEL = process.env.MATTE_MODEL
  || path.resolve(__dirname, "..", "assets", "ppmattingv2_stdc1_human_544x960.onnx");
const EMA = (() => { const v = parseFloat(process.env.MATTE_EMA || "0.5"); return v > 0 && v <= 1 ? v : 0.5; })();

function modelDims(modelPath) {
  const m = path.basename(modelPath).match(/(\d+)x(\d+)\.onnx$/i);
  if (!m) { console.error(`[matte] cannot parse <H>x<W> from model filename: ${modelPath}`); process.exit(3); }
...
function ensureModel() {
  ...
  console.error(`        skill, restore assets/ppmattingv2_stdc1_human_544x960.onnx or set MATTE_MODEL.`);
  process.exit(3);
}
...
async function main() {
  ...
  const W0 = meta.width, H0 = meta.height, N = W0 * H0;
  ...
  const rw = Math.max(1, Math.round(W0 * scale)), rh = Math.max(1, Math.round(H0 * scale));
  ...
  for (let i = startAt; i < files.length; i++) {
    ...
    const padded = await sharp(files[i]).removeAlpha()
      ...
      .extend({ top: oy, bottom: MH - rh - oy, left: ox, right: MW - rw - ox, background: { r: 0, g: 0, b: 0 } })
      .raw().toBuffer(); // RGB HWC uint8, MW×MH
    const M = MH * MW;
    const inp = new Float32Array(3 * M);
    for (let p = 0; p < M; p++) { inp[p] = padded[p * 3] / 255; inp[M + p] = padded[p * 3 + 1] / 255; inp[2 * M + p] = padded[p * 3 + 2] / 255; }
    ...
    await sharp(rgba, { raw: { width: W0, height: H0, channels: 4 } }).png().toFile(path.join(framesFg, path.basename(files[i])));
