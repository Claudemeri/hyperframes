...
grep '"type":"bgm"' assets/manifest.jsonl
```

Runnable: `node scripts/find-asset.mjs --workspace <dir> [--type image | --tag logo | --query "previous bgm"] [--human]` → matching AssetRecords as JSON (or tab-separated with `--human`). (A `hyperframes media find` CLI wrapper is still TBD with James — `provider-routing.md`.)
