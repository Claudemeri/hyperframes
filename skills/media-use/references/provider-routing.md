...
## free-first policy (Bin, 2026-06-04 review)
...
## CLI ownership (decided 2026-06-04)

- Capability APIs live in the **`heygen` CLI** (James) — install and use it; do not fork them into HyperFrames. Exception: cloud-rendering APIs live in the HyperFrames CLI.
- Whether media-use's own bookkeeping verbs (`find` / `organize` / `resolve` orchestration) become real `hyperframes media <verb>` subcommands, or stay skill procedures + thin helper scripts, is **open with James**.
...
- A per-skill **memory** of the user's provider preferences (James's suggestion, 2026-06-04), stored in the global workspace — so the agent recalls "use ElevenLabs for SFX" without re-deciding each time. Not in this prompt.
