#!/usr/bin/env node
// process — transform an existing asset (free / local first), write to assets/processed/,
...
//
// Usage:
//   node process.mjs --workspace <dir> --asset <id> --action remove-bg [--output assets/processed/foo.png]
...
if (action === "remove-bg" || action === "remove-background") {
  ...
  const rel = a.output && a.output !== true ? a.output : `assets/processed/${asset_id}${outExt}`;
  ...
  const rel = `assets/processed/${asset_id}.json`;
  const out = join(ws, rel);
  mkdirSync(dirname(out), { recursive: true });
