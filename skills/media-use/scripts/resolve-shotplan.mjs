...
// <project>/assets/ + an agent-readable index, then Director Part 2 designs AROUND them
...
//   --apply-decisions <file>    freeze the agent's picks into <project>/assets/ (media-use's own
...
//   <NAME>.mp4 + <NAME>-coords.json in assets/maps/, registers both with the tile-provider
...
function paletteSwatch(file, role) {
  try {
    const out = join(PROJECT, "assets/preview", `palette_${role}.png`);
    mkdirSync(dirname(out), { recursive: true });
    run("ffmpeg", ["-y", "-loglevel", "error", "-i", file, "-vf", "palettegen=max_colors=8:reserve_transparent=0", out]);
    return `assets/preview/palette_${role}.png`;
  } catch {
    return null;
  }
}
...
ensureWorkspace(PROJECT); // the host adopted media-use's own convention: <project>/assets/ + manifest + index
...
for (const d of decisions) {
  ...
  try {
    ...
    if (action.type === "bake") {
      ...
      const outDir = join(PROJECT, "assets/maps");
      ...
      const mp4 = `assets/maps/${name}.mp4`;
      const coords = `assets/maps/${name}-coords.json`;
...
if (applied.length) {
  ...
  const idx = join(PROJECT, "assets/index.md");
